...
// int io_getevents(aio_context_t ctx, long min_nr, long max_nr, struct io_event
// *events, struct timespec *timeout) { 	return syscall(__NR_io_getevents, ctx,
...
//     }
//     std::cout << "In async mode, the entry not in cache is " << not_in_cache
//     << std::endl;
...
//                 read_size = ALIGNMENT;
//             }
//             io_prep_pread(&io_req[prefix[n]], feature_fd, (void
//             *)(read_buffer + (ALIGNMENT * 2 * prefix[n]) / sizeof(float)),
...
//         request_stack.clear();
//         int submit_task = std::min(submit_batch, int(not_in_cache -
//         done_task)); request_stack.resize(submit_task); for (int i = 0; i <
//         submit_task; i ++) {
//             request_stack[i] = &io_req[i + done_task];
//         }
//         ret = io_submit(ctx, request_stack.size(),
//         reinterpret_cast<iocb**>(request_stack.data())); if (ret !=
//         request_stack.size()) {
//             if (ret < 0)
//                 printf("io_submit: %d : %s\n", ret, strerror(-ret));
//             else
//                 printf("could not sumbit IOs, submit number: %d, total:
//                 %d\n", ret, submit_task);
//             free(read_buffer);
//             free(result_buffer);
//             free(indice);
...
//             residual = offset - aligned_offset;
//             memcpy(result_buffer + feature_dim * n,
//                    read_buffer + (ALIGNMENT * 2 * prefix[n] + residual) /
